...
```bash
15_machinelearning_pytorch$ sam build
```
...
```bash
15_machinelearning_pytorch$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
15_machinelearning_pytorch$ sam local start-api
15_machinelearning_pytorch$ curl http://localhost:3000/classify_digit
```
...
```bash
15_machinelearning_pytorch$ sam logs -n InferenceFunction --stack-name "15_machinelearning_pytorch" --tail
```
...
```bash
sam delete --stack-name "15_machinelearning_pytorch"
```
