...
```bash
13_lambdaEFS$ sam logs -n HelloEfsFunction --stack-name "13_lambdaefs" --tail
```
...
```bash
13_lambdaEFS$ pip install -r tests/requirements.txt --user
# unit test
13_lambdaEFS$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
13_lambdaEFS$ AWS_SAM_STACK_NAME="13_lambdaefs" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "13_lambdaefs"
```
