...
CSACustom.main = function (application, marketplaceId, requestId, pageTypeInfo, enableAutoSPA = false) {

    if(application && marketplaceId && application != "undefined" && marketplaceId != "undefined"){
        ...
        if (window?.csa) {
            csa("Config", {
                ...
                'Events.SushiEndpoint': 'https://unagi.wholefoodsmarket.com/1/events/com.amazon.csm.csa.prod',
        ...
        var e = window?.document?.createElement("script"); e.src = "https://m.media-amazon.com/images/G/01/csa/dist/prod/v2/browser.external-analytics/async.min._TTH_.js"; window?.document?.head?.appendChild(e);
...
CSACustom.onTransitionSPA = function(){
    if(csa){
        let pageTypeInfo = CSACustom.getPageTypeInfoPIE()

        !CSACustom.getPageSlug()?.match(/\/product\//g) && csa("Events")("removeEntity", "product");
...
CSACustom.getCookie = function (key) {
    try {
        var match = window?.document?.cookie?.match(new RegExp('(^| )' + key + '=([^;]+)'));
...
CSACustom.getQueryParamValue = function (queryParam, key) {
    queryParam = (queryParam?.startsWith('?')) ? queryParam?.substring(1) : queryParam;
...
CSACustom.getPageTypeInfoQueryParam = function (queryParameter) {
    let queryParamVal =  CSACustom.getQueryParamValue(CSACustom.getURLQueryParameters(), queryParameter)
    ...
    (pageTypeInfo = {
        pageType: CSACustom.getPageSlug()+'_landing'
    })
}
...
CSACustom.getPageTypeInfoPIE = function () {
    ...
    if(slug?.match(/\/search/g)) {
        ...
        if (noFolders == 1){
            ...
            pageTypeInfo.subpageType = slug.substring(0,slug.indexOf('/', slug.indexOf('/', 1) + 1));
            pageTypeInfo.pageTypeId = slug;
        }
    }
    return pageTypeInfo;
...
CSACustom.getRequestId = function () {
    var requestId = CSACustom.generateUUID() + CSACustom.generateUUID() + CSACustom.generateUUID() + CSACustom.generateUUID()
    return requestId.toUpperCase().slice(0, 20)
}

CSACustom.setEvents = function () {
    let userAgent = navigator?.userAgent;
    let browserClient = CSACustom.getBrowser(navigator.appName, navigator.appVersion, userAgent);
    if (window?.csa) {
        csa("Events")("setEntity", {
            device: {
                userAgent: userAgent,
...
CSACustom.getDeviceType = function (userAgent) {
    ...
    let isTablet = userAgent && (/ipad|android|android 3.0|xoom|sch-i800|playbook|tablet|kindle|KFAPWI/i.test(userAgent.toLowerCase()));
    let isTabletWithMobile = userAgent && (/ipad|android 3.0|xoom|sch-i800|playbook|tablet|kindle|KFAPWI/i.test(userAgent.toLowerCase()));
...
CSACustom.getBrowser = function (appName, appVersion, userAgent) {
    let client = (() => {
        let engine = userAgent?.match(/(opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*(\d+)/i) || [],
            build;
